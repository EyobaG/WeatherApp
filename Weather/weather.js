...
const apikey = "d9866d2c4daf76fc80e09d874fe1ce47";
...
window.addEventListener("load", ()=>{
    if(navigator.geolocation){
        navigator.geolocation.getCurrentPosition((position)=>{
            ...
            const url = 'https://api.openweathermap.org/data/2.5/weather?lat={lat}&lon={lon}&appid=4202f9e0af7452884a34bec6ab8d2501';
...
function weatherReport(data){
    ...
    fetch(urlcast.then((res)=>{
        ...
        let iconur1= "http://api.openweathermap.org/img/w/"+ icon1 +".png";
        document.getElementById('img').src = iconur1
    })
}
...
function dayForecast(forecast){
    ...
    for (let i = 0; i < forecast.list.length; i+=8) {
        ...
        day.innerText = new Date(forecast.list[i].dl*1000).toDateString(undefined, 'Asia/Kolkata');
        div.appendChild(day);
